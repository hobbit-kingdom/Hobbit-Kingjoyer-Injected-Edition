v2.06.06
add:	[d3dumddi.cpp] handling of ALTPIXELCENTER flag
fix:	[ddraw.cpp] fixed handling of cooperative level in native mode
fix:	[ddraw.cpp] added renewBackBufferClipper procedure, adjusts the clipper area when set to the backbuffer surface
fix:	[ddraw.createsurface.cpp] added PLAINBACKBUFFER flag, fixes "Sci-Fi Pinball"
fix:	[ddraw.ddrawex.cpp] fixed handling of cooperative level for fullscreen modes
fix:	[dshow.* ole32.cpp] completely rewritten DirectShow wrapping (see help)
add:	[dsound.cpp] added EMULATESOUNDPAN flag to emulate PAN capability when not available
add:	[dxhelper.cpp] added more error codes
add:	[dxhook.cpp patcher.cpp] added ENABLEPATCHER flag
fix:	[dxwcore.cpp winproc.cpp] fixed window clipping handling
fix:	[excepthandler.cpp] added VGA interrupts needed for "Jane's Apache Longbow Gold"
fix: 	[flipoverlay.cpp] fixed flags, needed to fix IVMRSurfaceAllocator::AllocateSurface wrapper
add:	[guids.cpp] added more guids
fix:	[hd3d.cpp] fix - set the new screen size only when the backbuffer size is bigger ...
add:	[hd3d7.cpp] added FILTERRGBTEXTURES and NOD3DOPTIMIZE flags handling
add:	[hooktable.cpp] added d3dxof.dll hooking
fix:	[kernel32.cpp] added fakeCD/HD handling to WinExec wrapper
fix:	[mciplayer.cpp] fixed MCI_STOP handling
fix:	[mciwrapper.cpp] fixed MCI_SYSINFO and MCI_WHERE handling, added STRETCHMOVIE handling
add:	[opengl32.cpp] added COMPENSATEOGLCOPY and NOSCISSORTEST flags 
add:	[queryint.cpp] added FORCEDX1HOOK flag, needed for "Eryner"
add:	[savegraph.cpp] added GRAPHDUMP debug flag
add:	[sblit.cpp] added SURFACEUNLOCK flag, fixes "H.E.A.D.Z." missing heads
add:	[sdl.cpp] added SDL_ImgLoad wrapper and SDLFIXMOUSE flag
fix:	[user32.cpp] fixed mouse handling for fullscreen mode
add:	[user32.cpp] added "MainVXLIBWindow" active-movie window, fixes "Metro Police"
...
v2.06.04
add:	[comvsound.cpp] added fake devices for IDirectSoundBuffer8, improved fake DirectSound emulation
add:	[ddraw.cpp kernel32.cpp] added EMUDDSYNCSHIM flag for EmulateDirectDrawSync shim emulation
add:	[dumptext.cpp] added NAKED and TIMEDDUMP text dump modes
fix:	[dxhook.cpp dxwcore.cpp] fixed directory change when switching with fake drives inside the same real drive
add:	[dxhook.cpp] added RESETMULTIVOLUME flag
fix:	[gdi32.cpp] recovered and fixed text locale translation in TextOutA wrapper. Fixes "Top Blade V", "Quovadis"
fix:	[hd3d7.cpp] fixed invalid alpha value in SetRenderState(D3DRENDERSTATE_ALPHAREF). Fixes "Glover"
fix:	[hd3d7.cpp] fixed TRACETEXTURE compilation flag
fix:	[heap.cpp] improved SAFEHEAP handling, added REPAIRHEAP flag.
add:	[hooktable.cpp lz32.cpp] added lz32.dll hooking
add:	[kernel32.cpp] added multi-volume syntax for cdinfo.txt file for CDHACK flag
fix:	[kernel32.cpp] removed superfluos (and dangerous for Watcom libs) hook in LoadLibrary wrapper
fix: 	[kernel32.cpp] in GetVolumeInformation wrapper fixed logic when lpRootPathName is NULL but the current folder is a fake device. Fixes "Robot Commander" (Ch 1996) with fake-CD.
fix:	[kernel32] fixed GetLogicalDeviceStringsA/W wrappers to hide devices with HIDECDROMREAL flag
fix:	[kernel32.cpp] fixed SetThreadPriority, SuspendThread, ResumeThread and SetPriorityClass wrappers to reduce logging when emulating The IgnoreScheduler shim
fix:	[kernel32.cpp] dixed dirExists wrapper to handle paths with multiple slash terminators. Fixes "Zaphie" with fake-CD
fix:	[mciwrapper.cpp] fixed MciSendStringA/W wrapper for open command with syntax used by "Nakoruru ~Ano Hito kara no Okurimono~" (Jap 2001)
fix:	[mciwrapper.cpp] fixed MciSendStringA/W wrapper for pause command
fix: 	[shell32.cpp] fixed ShellExecuteA wrappers for NULL lpDirectory case. T.B.D. WIDE fix.
fix:	[filesys.cpp] fixed DxWnd internal status for multi-volume CD handling
fix: 	[filesys.cpp] fixed dxwTranslatePathA/W internal routines
add:	[winmm.cpp] added mmioSetInfo tracing wrapper
fix:	[winproc.cpp] fixed memory allocation bug in CD changer routine
add:	[GUI] added global configuration flags for naked dump and timed dump
add:	[GUI] enlarged status window to show also the fake multi-CD volume
...
fix:	[dxwnd.cpp] fixed WindowHook path comparison logic
...
fix:	[sblit.cpp ddraw.cpp] changed pGetSurfaceDescMethod prototype
add: 	[kernel32.cpp] added HIDECDROMREAL flag
add:	[kernel32.cpp] added support for CDRom labels in localized UTF8 charset by hex definition
fix:	[kernel32.cpp] fixed SearchPathA/W wrappers to handle NULL path condition
add: 	[dxwcore.cpp] added OUTVGA log topic
fix: 	[dwflip.cpp] fixed condition where a surface may appear more than once in the flipchain list
fix:	[ddcreates.cpp]	improved handling of PRIMARY_FLIPPABLE mode to avoid creation of unused backbuffers
...
add:	[tools\getcaps.c] tool to report some of the device capabilities
fix:	[GUI] fixed "Clear compatibility flags" menu command forcing KEY_WOW64_64KEY

v2.05.97
fix:	[comdlg32.cpp] added handling of fake HD/CD in GetOpenFileNameA/W proxies, added more complete log.
fix:	[ddraw.cpp] fixed DumpPixelFormat logging utility
fix:	[dsound.cpp] returning correct value in DirectSoundCreate8 wrapper
add:	[dxhook.cpp, dxwcore.cpp, dxwcore.hpp, dxwnd.cpp] added 4 more flag DWORDs
fix:	[gdi32.cpp] fixed GetDeviceCaps wrapper in handling of NUMCOLORS and NUMRESERVED queries
add:	[hd3d.cpp] added tracing proxies for D3D8/9 SetCursorProperties methods
fix: 	[hd3d.cpp] in HookDirect3D don't alter hook version 8 or greater
fix:	[hd3d.cpp, hd3d7.cpp] fixed wrong activation for FORCEDITHERING and CLEARDITHERING flags
add: 	[hd3d.cpp, hd3d8.cpp] added MERGEFRONTBUFFERS flags. Fixes "The Legend Of Dragon's Son" shared usage of front buffers
add: 	[hd3d.cpp] added flags to force the device type in Direct3DDevice8/9 wrappers
fix:	[hd3d7.cpp] moved Debug flags PATCHEXECUTEBUFFER, FORCEZBUFFERON, FORCEZBUFFEROFF, FORCEDITHERING, CLEARDITHERING to Direct3D(2) tab.
fix:	[heap.cpp] reduced HEAPPADSIZE value from 10000 to 1000
add:	[kernel32.cpp] added FIXOVERLAPPEDRESULT flag and GetOverlappedResult wrapper. Fixes "David Douillet Judo".
add:	[kernel32.cpp] added GlobalGetAtomName tracing wrapper
add:	[kernel32.cpp] added CopyFileA/W wrappers and handling of fake CD/HD
add:	[kernel32.cpp] ifdef-ed tracng of Tls calls (TlsAlloc, TlsFree, ...)
fix:	[kernel32.cpp] fixed log message in SearchPathA/W wrappers
fix: 	[mciwrapper.cpp] handling of naked "open cdaudio" command found in "XTom 3D"
add:	[mss32.cpp] added FIXAILBUG flag, used for "Brave Hart"
add:	[user32.cpp] added ModifyMenuA wrapper
add:	[user32.cpp] added EMULATEWINHELP flag
fix:	[vfilesys.cpp] fixed handling of files within the current path
add:	[winproc.cpp] added LOCKCURSORSHAPE flag
fix:	[GUI] revised interface and help pages
...
v2.05.93
add:	[nogameux] proxy dll and registry scripts to fix the gameux shim problem in Win7
fix:	[createproc.cpp] added handling of fake CD/HD in program path
add:	[dwflip.cpp, ddcreates.cpp, ddraw.cpp, dxwstack.cpp, sblit.cpp] added handling of flipchain with multiple backbuffer surfaces
add:	[excepthandler.cpp] added FORCENOPS flag handling
fix:	[gdi32.cpp] generalized detection of a real or virtual desktop window in DescribePixelFormat wrapper
fix:	[kernel32.cpp] fixed log messages causing exceptions in GetVolumeInformationA/W wrappers
fix:	[mciwrapper.cpp] fixed log messages causing exceptions in mciSendCommand wrapper
fix:	[shell32.cpp] fixed ShellExecuteA wrapper when running programs in a subfolder
fix:	[user32.cpp] added locale translation in AppendMenuA wrapper
fix:	[user32.cpp] fixed black topmost window in "the three swordsmen" game
fix:	[user32] fixed stretching of dialog in DialogBoxParamA wrapper
fix:	[vfilesys.cpp] fixed path translation in both cases where folder terminates with "/" or "\"
add:	[wing32.cpp] added hdc and bitmap dump debug option to wing32 calls
add:	[winmm.cpp] added wrappers for mmio/midi calls mmioDescend, mmioSeek, mmioRead, midiStreamPosition
add:	[winproc.cpp] added handling of invert mouse axis in non-windowed mode
add:	[libogg, libvorbis] added obb/vorbis projects compatible with WinXP-Win11 systems
...
v2.05.91
fix: [cdaproxy\mciwrapper.cpp] aligned to mciwrapper.cpp changes
...
v2.05.83.fx4
fix: [ddblit.cpp] fixed logging messages telling difference between sync and async mode
fix: [ddraw.cpp] fixed IDIrectDrawSurface::Release wrapper. Avoids clearing emulation surfaces if a primary surface is still alive.
fix: [ddcreates.cpp] fixed exception in primary surface creation when the previous ddraw session was not properly cleaned up. Fixes exception in "Dark Secrets of Africa" and others.
add: [guids.cpp] added a few more GUIDs
...
v2.05.79
add: [cdaproxy\OutTrace.cpp] added possibility to insert timestamp in the cdaproxy logs
...
fix: [tools\bchunk.cpp] better logic to match .cue to .bin files
add: [tools\keymap.cpp] dump keyboard type info
...
fix: [tools\bchunk\bchunk.cpp] fixed help text and upper/lowercase pathname handling
...
v2.05.60
add: [comdlg32.cpp] wrappers to bypass stretching in PrintDlg system calls
fix: [mciwrapper.cpp] in mciSendString seek command returns a DWORD (or in any case not a valid string)
fix: [mciwrapper.cpp] in mciSendCommand wrapper on MCI_PUT command restore original RECT only when necessary. Fixes a possible memory violation exception.
fix: [advapi.cpp] fixed wrappers for extRegCreateKey/Ex to get existing fake keys when configured. Fixes a problem with "Beast Wars: Transformers"
fix: [user32.cpp] fixed SetWindowPos wrapper handling of window Z-order. Fixes "Virus the Game"
fix: [winmm.cpp] fixed a bug about quoted paths in mciSendString wrapper with fake CD/HD
fix: [user32.cpp] suppressed rect scaling in DrawText* wrappers when DT_CALCRECT is set. Fixes "Corum Side Story" text clipping
add: [winmm.cpp] added MIDIAUTOREPAIR flag: performs recovery actions when midi calls fail
add: [dxemublt.cpp] added dithering simulation filter
add: [user32.cpp] added LEGACYBASEUNITS flag: trims the result of GetDialogBaseUnits to a maximum value of 16x8 pixels
add: [vbasic.cpp] tentative patcher for vb6 initialization activated by "patch:PatchMSVBVM" tweak

v2.05.59
add: [sdl.cpp] added wrappers for fake CD to SDL SDL_CDNumDrives and SDL_CDName. Fixes russian SDL version of "Petka 3"
fix: [winmm.cpp] fixed logging of mixerGetLineControlsA. Seen in "Twisted Metal 2"
fix: [dxwndhostView.cpp] proposed export file path cleaned from invalid characters in pathname
fix: [mciwrapper.cpp] added emulation for MCI_STATUS MCI_STATUS_CURRENT_TRACK case in mciSendCommand wrapper. Fixes "Mastermind".
fix: [mciwrapper.cpp] added delay for synchronization of immediate MCI_GET. Fixes "Rockman Legends".
fix: [***] revised code for CD changer functionality, needed for "Master of Dimensions".
add: [user32.cpp] added CENTERDIALOGS "win:CenterDialogs" tweak. Useful for "Saga: Rage of the Vikings".
add: [dxdiaghook.cpp] added DXVERSIONLIE tweak, same as DirectXVersionLie MS shim. Fixes "Liquidator 2".
fix: [kernel32.cpp] fixed extended injection of child processes when process has no .exe extension in the path. Fixes "Ultimate Pinball Extreme".
fix: [kernel32.cpp] fixed create process with debug injection to continue running in case of continuable exceptions
fix: [mciwrapper.cpp] fixed MCI_PLAY command issued in two parts, MCI_FROM followed by MCI_TO. Fixes "Entomorph Plague of the Darkfall".

v2.05.58
fix: [winmm.cpp] added fake capabilities for aux CD port SB16-like
fix: [winmm.cpp] revised (hopefully improved?) handling of aux device ids
add: [winmm.cpp] added winmm:auxOutMessage wrapper for tracing
fix: [kernel32.cpp] fixed dxwUntranslatePathW bugs. Fixed "Petka 3" with fake-CD
fix: [winmm.cpp] handled possible recursion when changing CD volume for mp3 and wav formats using mciSendString
fix: [mciwrapper.cpp] added tentative handling of "status cdaudio position" in mciSendString wrapper
fix: [mciwrapper.cpp] suppressed MCI_OPEN_SHAREABLE flag when opening CD drives mapped to virtual CD folders. Found in "Twisted Metal 2".
fix: [mciwrapper.cpp] fixed handling of file paths in mciSendStrings that, because of virtual CD mapping, become needing double-quotes incapsulation.
fix: [hd3d7.cpp] protected d3d BeginScene wrappers when using a viewport NULL reference
fix: [dxwsstack.cpp] fixed logic to recycle entries in surface table. Fixes "Nocturne" loads.
add: [winmm.cpp] added HIDEMUTECONTROLS flag. Fixes "Snow Wave Avalanche" bugs.

v2.05.57
fix: [mciwrapper.cpp] now dxwLockMciDeviceId auto-generates the dxwnd.wav file with minimal size
fix: [winmm.cpp dxwcore.cpp] mixer volume for CD audio control now saved on disk and recovered on next run
add: [winmm.cpp] FIGHTINGFORCEFIX flag: changes the CD audio mixer control shortname to match "Fighting Force" expectations
fix: [winmm.cpp] split handling of CD audio mci id and mixer id, whatever it means ...
fix: [mciwrapper.cpp] unified handling of CD audio mci id through the dxwLockMciDeviceId() routine
add: [cdaproxy] merged cdaudioproxy project in DxWnd project
